...
type ManagedSmtpFleetHealthRead = {
  ...
  primary_next_action_summary: string;
  primary_next_action_tone: 'good' | 'warn';
...
function DeliveryPage({ sendJobs, sendRecords, campaigns, route, onRefresh, onOperation }: {
  ...
  const [mtaIpPoolForm, setMtaIpPoolForm] = useState({
    max_per_minute: '',
    min_available_nodes: '',
    status: '',
  });
  const [mtaIpPoolNodes, setMtaIpPoolNodes] = useState<MtaIpPoolNodeRead[]>([]);
  const [mtaIpPoolNodeTotal, setMtaIpPoolNodeTotal] = useState(0);
  const [selectedMtaIpPoolNodeId, setSelectedMtaIpPoolNodeId] = useState('');
  const [mtaIpPoolNodeForm, setMtaIpPoolNodeForm] = useState({
    max_per_minute: '',
    priority: '',
    weight: '',
    status: '',
  ...
  const managedSmtpFleetItems = [
    ...
    },
    {
      ...
      value: managedSmtpDeploymentSummary
        ...
        : 'Unknown',
      detail: managedSmtpDeploymentSummary
        ? managedSmtpDeploymentSummary.fleet_health.primary_next_action_summary
        : 'Load deployment summary for fleet action guidance.',
      tone: managedSmtpDeploymentSummary?.fleet_health.primary_next_action_tone || 'good',
    },
    {
  ...
  function exportManagedSmtpRouteMatrixCsv() {
    if (!managedSmtpRouteMatrix) {
      setStatus('Run the resolver matrix before exporting CSV.');
      return;
    }
    const rows = [
      ['Managed SMTP Resolver Matrix Export'],
      ['Generated at', new Date().toISOString()],
      ['Domain policy', selectedDomainPolicy?.domain || ''],
      ['Route ID', selectedDomainPolicy?.route_id || ''],
      ...
      ...managedSmtpRouteMatrix.results.map((item) => {
        ...
        return [
          ...
          item.result.ok ? 'ready' : 'blocked',
          route?.routing_rule_name || (route ? 'Default policy' : ''),
          route?.ip_pool_name || '',
          route?.mta_node_name || '',
          route?.provider || '',
          reason?.code || '',
    ...
    link.href = url;
    link.download = `managed-smtp-resolver-matrix-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
    URL.revokeObjectURL(url);
    setStatus(`Downloaded managed SMTP resolver matrix CSV with ${formatInt(managedSmtpRouteMatrix.total)} row(s).`);
  }
  ...
  function buildManagedSmtpFleetHealthEvidencePack() {
    ...
    const nodes = (summary?.recent_nodes || []).slice(0, 8).map((item) => [
      `- node=${item.node.name}`,
      `  host=${item.node.hostname}`,
      `  status=${item.node.status}`,
      `  operational=${item.agent_operational_status_label || item.agent_operational_status}`,
      `  route_ready=${item.readiness_summary.latest_success ? 'true' : 'false'}`,
      `  latest_readiness=${item.readiness_summary.latest_check?.status || '-'}`,
      `  heartbeat=${item.agent_heartbeat_status_label}`,
      `  service=${item.agent_service_state_label || '-'}`,
      `  timer=${item.agent_timer_state_label || '-'}`,
      `  config=${item.agent_config_sync_label}`,
      `  revision=${item.agent_code_revision_label}`,
      `  host_update=${item.agent_host_update_status_label}`,
    ...
    return [
      ...
      `Operational: ok=${formatInt(fleet?.operational_ok_nodes || 0)} warning=${formatInt(fleet?.operational_warning_nodes || 0)} blocked=${formatInt(fleet?.operational_blocked_nodes || 0)}`,
      `Agent coverage: stale=${formatInt(fleet?.stale_agent_nodes || 0)} missing=${formatInt(fleet?.missing_agent_nodes || 0)} config_drift=${formatInt(fleet?.config_drift_nodes || 0)}`,
      `Host updates: required=${formatInt(fleet?.host_update_required_nodes || 0)} code_missing=${formatInt(fleet?.code_missing_nodes || 0)} code_dirty=${formatInt(fleet?.code_dirty_nodes || 0)} code_outdated=${formatInt(fleet?.code_outdated_nodes || 0)}`,
      `Systemd: service_failed=${formatInt(fleet?.agent_service_failed_nodes || 0)} timer_unhealthy=${formatInt(fleet?.agent_timer_unhealthy_nodes || 0)}`,
  ...
  async function copyManagedSmtpFleetHealthEvidencePack() {
    if (!managedSmtpDeploymentSummary) {
      setStatus('Load managed SMTP deployment summary before copying a fleet health pack.');
      return;
    }
    await copyTextToClipboard(buildManagedSmtpFleetHealthEvidencePack());
  ...
  function mtaIpPoolFormFromPool(pool: MtaIpPoolRead | null) {
    return {
      max_per_minute: pool?.max_per_minute ? String(pool.max_per_minute) : '',
      min_available_nodes: pool?.min_available_nodes ? String(pool.min_available_nodes) : '',
      status: pool?.status || '',
    };
  }

  function mtaIpPoolNodeFormFromMembership(membership: MtaIpPoolNodeRead | null) {
    return {
      max_per_minute: membership?.max_per_minute ? String(membership.max_per_minute) : '',
      priority: membership ? String(membership.priority) : '',
      weight: membership ? String(membership.weight) : '',
      status: membership?.status || '',
    };
  }

  function updateMtaIpPoolForm(name: keyof typeof mtaIpPoolForm, value: string) {
    setMtaIpPoolForm((current) => ({ ...current, [name]: value }));
  }
  ...
  function loadManagedSmtpRoutingRuleIntoForm(rule: Record<string, unknown>) {
    ...
    setSelectedManagedSmtpRoutingRuleName(ruleName);
    setManagedSmtpRoutingRuleForm({
      name: ruleName,
      priority: String(rule.priority ?? '100'),
      enabled: rule.enabled !== false,
      send_types: listValue(rule.send_types),
      sender_domains: listValue(rule.sender_domains),
  ...
  async function loadManagedSmtpPoolControls() {
    await runDeliveryOperation('Loading managed SMTP pool controls', async () => {
      ...
      const nextPool = pools.find((pool) => pool.id === selectedMtaIpPoolId) || pools[0] || null;
      const nextMembership = memberships.find((membership) => membership.id === selectedMtaIpPoolNodeId)
        || memberships.find((membership) => membership.ip_pool_id === nextPool?.id)
        || memberships[0]
        || null;
      setMtaIpPools(pools);
  ...
  async function saveManagedSmtpPoolControls() {
    await runDeliveryOperation('Saving managed SMTP pool controls', async () => {
      if (!selectedMtaIpPool) throw new Error('Select an IP pool.');
      const updated = await fetchJson<MtaIpPoolRead>(`/api/v1/managed-smtp/ip-pools/${selectedMtaIpPool.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
      ...
      const summary = await fetchJson<ManagedSmtpDeploymentSummaryRead>('/api/v1/managed-smtp/deployment-summary?limit=8');
      setMtaIpPools((items) => items.map((item) => item.id === updated.id ? updated : item));
      setMtaIpPoolForm(mtaIpPoolFormFromPool(updated));
      setManagedSmtpDeploymentSummary(summary);
      return `Saved pool controls for ${updated.name}: ${updated.max_per_minute ? `${formatInt(updated.max_per_minute)}/min` : 'no rate gate'}, ${updated.min_available_nodes ? `${formatInt(updated.min_available_nodes)} required node(s)` : 'default capacity'}.`;
    });
  }

  async function saveManagedSmtpPoolNodeControls() {
    await runDeliveryOperation('Saving managed SMTP pool membership controls', async () => {
      if (!selectedMtaIpPoolNode) throw new Error('Select a pool membership.');
      const updated = await fetchJson<MtaIpPoolNodeRead>(`/api/v1/managed-smtp/ip-pool-nodes/${selectedMtaIpPoolNode.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
          max_per_minute: optionalPositiveInt(mtaIpPoolNodeForm.max_per_minute, 'Pool node max per minute'),
      ...
      const summary = await fetchJson<ManagedSmtpDeploymentSummaryRead>('/api/v1/managed-smtp/deployment-summary?limit=8');
      setMtaIpPoolNodes((items) => items.map((item) => item.id === updated.id ? updated : item));
      setMtaIpPoolNodeForm(mtaIpPoolNodeFormFromMembership(updated));
      setManagedSmtpDeploymentSummary(summary);
      return `Saved pool membership controls for ${mtaNodeNameForPoolMembership(updated)}: ${updated.max_per_minute ? `${formatInt(updated.max_per_minute)}/min` : 'no node rate gate'}.`;
    });
  }
  ...
  async function setSelectedManagedSmtpRoutingRuleEnabled(enabled: boolean) {
    await runDeliveryOperation(`${enabled ? 'Enabling' : 'Disabling'} managed SMTP routing rule`, async () => {
      ...
      setManagedSmtpRoutingRules(data);
      setManagedSmtpRoutingRuleForm((current) => ({ ...current, enabled }));
      return `${enabled ? 'Enabled' : 'Disabled'} routing rule ${ruleName}.`;
    });
  }
  ...
  async function deleteSelectedManagedSmtpRoutingRule() {
    await runDeliveryOperation('Deleting managed SMTP routing rule', async () => {
      ...
      setSelectedManagedSmtpRoutingRuleName('');
      setManagedSmtpRulePreview(null);
      return `Deleted routing rule ${ruleName}; ${formatInt(data.rules.length)} rule(s) remain.`;
    });
  }
    ...
    <section className="page-grid">
      ...
      <section className="panel full-span provider-feedback-panel">
        ...
        <div className="form-grid compact-form">
          <label className="wide-field">
            IP pool
            <select value={selectedMtaIpPoolId} onChange={(event) => {
              const pool = mtaIpPools.find((item) => item.id === event.target.value) || null;
              const membership = mtaIpPoolNodes.find((item) => item.ip_pool_id === pool?.id) || null;
              ...
              if (membership) {
                ...
                setSelectedMtaIpPoolNodeId('');
                setMtaIpPoolNodeForm(mtaIpPoolNodeFormFromMembership(null));
              }
            }}>
              <option value="">Select pool</option>
              {mtaIpPools.map((pool) => (
                <option value={pool.id} key={pool.id}>
                  {pool.name} | {pool.pool_type} | {pool.status}
                </option>
              ))}
            </select>
          </label>
          <label>
            Pool status
            <select value={mtaIpPoolForm.status} onChange={(event) => updateMtaIpPoolForm('status', event.target.value)}>
          ...
          <label>
            Pool max / min
            <input value={mtaIpPoolForm.max_per_minute} onChange={(event) => updateMtaIpPoolForm('max_per_minute', event.target.value)} placeholder="empty disables" inputMode="numeric" />
          </label>
          <label>
            Required nodes
            <input value={mtaIpPoolForm.min_available_nodes} onChange={(event) => updateMtaIpPoolForm('min_available_nodes', event.target.value)} placeholder="default 1" inputMode="numeric" />
          </label>
          <button className="ghost" type="button" onClick={saveManagedSmtpPoolControls} disabled={busy || !selectedMtaIpPoolId}>Save Pool</button>
          <label className="wide-field">
            Pool membership
            <select value={selectedMtaIpPoolNodeId} onChange={(event) => {
              const membership = mtaIpPoolNodes.find((item) => item.id === event.target.value) || null;
              const pool = membership ? mtaIpPools.find((item) => item.id === membership.ip_pool_id) || null : null;
              setSelectedMtaIpPoolNodeId(membership?.id || '');
              setMtaIpPoolNodeForm(mtaIpPoolNodeFormFromMembership(membership));
              if (membership) {
                setSelectedMtaIpPoolId(membership.ip_pool_id);
                setMtaIpPoolForm(mtaIpPoolFormFromPool(pool));
              }
            }}>
              <option value="">Select membership</option>
              {mtaIpPoolNodes
                .filter((membership) => !selectedMtaIpPoolId || membership.ip_pool_id === selectedMtaIpPoolId)
                .map((membership) => (
                  <option value={membership.id} key={membership.id}>
                    {mtaNodeNameForPoolMembership(membership)} | priority {membership.priority} | {membership.status}
                  </option>
                ))}
            </select>
          </label>
          <label>
            Membership status
            <select value={mtaIpPoolNodeForm.status} onChange={(event) => updateMtaIpPoolNodeForm('status', event.target.value)}>
              <option value="">current</option>
              <option value="active">active</option>
              <option value="paused">paused</option>
              <option value="draining">draining</option>
          ...
          <label>
            Node max / min
            <input value={mtaIpPoolNodeForm.max_per_minute} onChange={(event) => updateMtaIpPoolNodeForm('max_per_minute', event.target.value)} placeholder="empty disables" inputMode="numeric" />
          </label>
          <label>
            Priority
            <input value={mtaIpPoolNodeForm.priority} onChange={(event) => updateMtaIpPoolNodeForm('priority', event.target.value)} placeholder="100" inputMode="numeric" />
          </label>
          <label>
            Weight
            <input value={mtaIpPoolNodeForm.weight} onChange={(event) => updateMtaIpPoolNodeForm('weight', event.target.value)} placeholder="100" inputMode="numeric" />
          </label>
            ...
            {managedSmtpDeploymentSummary.recent_nodes.map((item) => (
              <article className={item.operator_next_action_tone || (item.agent_operational_status === 'ok' ? 'good' : 'warn')} key={item.node.id}>
                ...
                <dl>
                  ...
                  <div><dt>action code</dt><dd>{item.operator_next_action_code_label || 'Unknown'} / {item.operator_next_action_code || 'unknown'}</dd></div>
        ...
        <div className="managed-smtp-route-inspector">
          ...
          {(managedSmtpRoutingRules?.rules || []).map((rule) => (
            <article
              ...
              key={String(rule.name || rule.id || JSON.stringify(rule))}
              onClick={() => loadManagedSmtpRoutingRuleIntoForm(rule)}
            >
              <span>{String(rule.name || 'unnamed rule')}</span>
              <strong>{String(rule.ip_pool_name || rule.ip_pool || rule.mta_ip_pool_id || '-')}</strong>
              <small>{`${rule.enabled === false ? 'disabled' : 'enabled'} / priority ${String(rule.priority ?? '-')} / ${Array.isArray(rule.preferred_providers) ? rule.preferred_providers.join(', ') : 'any provider'}`}</small>
            </article>
          ))}
    ...
    <section className="page-grid">
      ...
      <section className="panel full-span campaign-workbench">
        ...
        <div className="analytics-foundation-panel">
          ...
          <div className="analytics-foundation-strip">
            {analyticsFoundationSignals.map((item) => (
              <article className={`analytics-foundation-item ${item.tone}`} key={item.label}>
                <span>{item.label}</span>
                <strong>{item.value}</strong>
                <small>{item.detail}</small>
              </article>
