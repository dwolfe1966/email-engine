...
- `POST /api/v1/campaigns/{campaign_id}/launch` creates durable campaign send jobs and queued send records. It does not send the queued records yet; worker-based delivery is the next backlog slice.
