...
def test_openapi_exposes_gui_integration_paths() -> None:
    ...
    expected_paths = {
        ...
        '/api/v1/delivery-routes/{route_id}/managed-smtp/routing-rules',
        '/api/v1/delivery-routes/{route_id}/managed-smtp/routing-rules/{rule_name}',
        '/api/v1/delivery-routes/{route_id}/managed-smtp/routing-rules/{rule_name}/disable',
        '/api/v1/delivery-routes/{route_id}/managed-smtp/routing-rules/{rule_name}/enable',
        '/api/v1/delivery-routes/{route_id}/managed-smtp/routing-rules/promotion/activate',
        '/api/v1/delivery-routes/{route_id}/managed-smtp/routing-rules/promotion/draft',
        '/api/v1/delivery-routes/{route_id}/managed-smtp/routing-rules/promotion/preview',
...
def test_delivery_attempt_evidence_summary_schema_exposes_rollups() -> None:
    ...
    assert {
        ...
        'top_rule_sources',
        'top_pool_sources',
        'top_provider_preferences',
        'top_provider_preference_modes',
        'top_provider_fallbacks',
        'top_provider_preference_blocks',
        'top_provider_fallback_availability',
        'top_block_codes',
        'top_block_messages',
        'top_node_candidate_counts',
        'top_node_skipped_counts',
        'top_node_selection_priorities',
        'top_node_selection_weights',
        'top_submission_providers',
        'top_submission_hosts',
        'top_submission_ports',
        'top_mta_hostnames',
...
def test_delivery_attempt_evidence_export_includes_provider_preference_columns() -> None:
    ...
    for token in [
        ...
        "metadata.get('route_mode_mta_ip_pool_id')",
        ...
        "metadata.get('mta_hostname')",
        "metadata.get('mta_pool_capacity_status')",
        "metadata.get('mta_pool_available_node_count')",
        "metadata.get('mta_pool_required_available_node_count')",
        "metadata.get('mta_rate_limit_scope')",
        "metadata.get('mta_rate_limit_window_seconds')",
        "metadata.get('mta_rate_limit_max_per_minute')",
        "metadata.get('mta_rate_limit_recent_count')",
...
def test_ai_campaign_analysis_accepts_scaleway_direct_proof_route_context() -> None:
    client = TestClient(app)
    response = client.post(
        '/api/v1/ai/campaigns/analyze',
        json={
            'campaign_context': {
                'campaign': {'id': 'campaign-a', 'name': 'Scaleway direct test', 'status': 'draft'},
                'template': {'id': 'template-a', 'name': 'Direct route template'},
                'validation': {'ok': True, 'errors': [], 'warnings': []},
                'audience': {'id': 'audience-a', 'name': 'Warm leads', 'estimated_count': 250},
                'performance': {'sent_count': 0, 'opened_count': 0, 'clicked_count': 0},
                'workflow_status': {
...
def test_ai_delivery_analysis_accepts_scaleway_direct_selected_job_route() -> None:
    client = TestClient(app)
    response = client.post(
        '/api/v1/ai/delivery/analyze',
        json={
    ...
    assert any(expected_summary == item for item in data['summary'])
    codes = {item['code'] for item in data['recommendations']}
    assert 'resolve_selected_job_route' not in codes
