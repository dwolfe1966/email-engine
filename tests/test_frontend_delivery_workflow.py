from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
FRONTEND_SOURCE = ROOT / 'frontend' / 'src' / 'main.tsx'
FRONTEND_DIST = ROOT / 'frontend' / 'dist'


def frontend_source() -> str:
    return FRONTEND_SOURCE.read_text()


def frontend_bundle() -> str:
    assets = sorted((FRONTEND_DIST / 'assets').glob('index-*.js'))
    assert assets, 'frontend/dist does not include the built ESP index bundle'
    return '\n'.join(asset.read_text() for asset in assets)


def assert_delivery_triage_contract(source: str) -> None:
    expected_tokens = [
        'deliveryTriageAction',
        'deliveryTriageItems',
        "title: 'Review failed records'",
        "title: 'Process queued delivery'",
        "title: 'Load job progress'",
        "title: 'Delivery clear'",
        'Delivery triage',
        'Queue pressure',
        'Failure review',
        'Selected job',
        'Selected record',
        'selectedJobProofRoute',
        'selectedJobProofRouteDetail',
        "selectedJob?.metadata_json?.latest_proof_route",
        'Job proof route',
        'selected_job_proof_route_evidence',
        'selectedRecordRouteAttempt',
        'selectedRecordRouteEvidence',
        'selectedRecordRouteEvidenceDetail',
        'selected_record_latest_attempt',
        'selected_record_route_evidence',
        'Record route',
        'aiSelectedRecordRouteAttempt',
        'aiSelectedRecordRouteEvidence',
        'items.find(attemptHasRouteEvidence)',
        'providerFootprint',
        'deliveryFoundationItems',
        'Send Engine Foundation',
        'Managed MTA nodes',
        'Load deployment summary or apply a provider profile to populate MTA inventory.',
        'Send queues',
        'Bounce queues',
        'No failed-record bounce signals in the loaded list.',
        'Deliverability feedback',
        'Provider events and managed-MTA feedback are normalized into one delivery evidence stream.',
        'deliveryOperationsContractItems',
        'Send Engine Operations Contract',
        'Operational contracts for managed MTA routing, queue lifecycle, retries, bounces, feedback, and deliverability controls.',
        'SMTP service contract',
        'Apply the Scaleway profile or load deployment summary to inspect managed-MTA route coverage.',
        'Queue lifecycle',
        'Send queues need claim locks, visibility timeouts, dead-letter handling, and operator-safe replay controls.',
        'Queue audit',
        'claim-blocked audit row(s)',
        'delivery-audit-panel',
        'delivery-audit-list',
        'Delivery Attempt Audit',
        'Load Attempt Audit',
        'Dead-letter Record',
        '/api/v1/delivery-attempts/list',
        '/api/v1/email-send-records/${selectedRecordId}/dead-letter',
        'ProviderFeedbackEventRead',
        'ManagedSmtpReadinessCheckRead',
        'ManagedSmtpDeploymentSummaryRead',
        'ManagedSmtpFirstSendRead',
        'MtaNodeEventRead',
        'providerFeedbackEvents',
        'providerFeedbackTotal',
        'mtaNodeEvents',
        'mtaNodeEventTotal',
        'mtaNodeEventFilters',
        'buildMtaNodeEventEvidencePack',
        'copyMtaNodeEventEvidencePack',
        'buildRuntimeConfigApplyEvidencePack',
        'copyRuntimeConfigApplyEvidencePack',
        'exportRuntimeConfigApplyEvidenceCsv',
        'exportMtaNodeEventEvidenceCsv',
        'MTA Agent Telemetry Evidence Pack',
        'Runtime Config Apply Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Copied runtime config apply evidence pack',
        'Downloaded runtime config apply CSV',
        'Copied MTA agent telemetry evidence pack',
        'Downloaded MTA agent telemetry CSV',
        'managedSmtpDeploymentSummary',
        'managedSmtpDeploymentItems',
        'managedSmtpFleetItems',
        'buildManagedSmtpDeploymentEvidencePack',
        'copyManagedSmtpDeploymentEvidencePack',
        'Managed SMTP Deployment Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Copied managed SMTP deployment evidence pack',
        'buildManagedSmtpFleetHealthEvidencePack',
        'copyManagedSmtpFleetHealthEvidencePack',
        'Managed SMTP Fleet Health Evidence Pack',
        'Copied managed SMTP fleet health evidence pack',
        'buildManagedSmtpBootstrapProfileEvidencePack',
        'copyManagedSmtpBootstrapProfileEvidencePack',
        'Managed SMTP Bootstrap Profile Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Copied managed SMTP bootstrap profile evidence pack',
        'AWS host setup',
        'Install/start email-engine-mta-agent on the AWS MTA host',
        'AWS DNS/rDNS checklist',
        'mta-aws-001.email-engine.app',
        'returns-aws.email-engine.app',
        'ee-aws1',
        'buildAwsDnsActivationEvidencePack',
        'copyAwsDnsActivationEvidencePack',
        'AWS DNS Activation Evidence Pack',
        'Copy AWS DNS Pack',
        'Copied AWS DNS activation evidence pack',
        'AWS next gate',
        'AWS next gate:',
        'Complete DNS/rDNS',
        'Verify DNS/rDNS',
        'Run controlled seed',
        'updateAwsProviderRdnsStatus',
        'verifyAwsProviderRdns',
        'Verify AWS rDNS',
        'AWS rDNS verification',
        '/verify-rdns',
        'AWS rDNS verification ',
        'Domain auth verification',
        'verifySelectedDomainAuthentication',
        'Verify Domain Auth',
        '/verify-authentication',
        'Domain authentication verification',
        'Mark AWS rDNS Configured',
        'Reopen AWS rDNS',
        '/api/v1/managed-smtp/provider-accounts/',
        'Updated AWS provider rDNS status',
        'buildManagedSmtpProviderReadinessEvidencePack',
        'copyManagedSmtpProviderReadinessEvidencePack',
        'Managed SMTP Provider Readiness Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Copied managed SMTP provider readiness evidence pack',
        'AWS activation signal',
        'AWS activation:',
        'AWS instance setup signal',
        'AWS agent setup:',
        'Log into AWS instance',
        'AWS agent setup commands',
        'AWS MTA Agent Setup Evidence Pack',
        'buildAwsAgentSetupEvidencePack',
        'copyAwsAgentSetupEvidencePack',
        'Copy AWS Agent Pack',
        'MANAGED_SMTP_MTA_NODE_ID',
        'MANAGED_SMTP_FEEDBACK_SECRET',
        'python3 scripts/managed_smtp_mta_agent.py --post-config-event --json',
        'delivery-score-strip',
        'Managed SMTP deployment score strip',
        'Managed SMTP fleet health strip',
        'fleet_health',
        'platform_code_revision',
        'platform_code_revision_label',
        'Fleet status',
        'Route-ready nodes',
        'Node status',
        'operational_ok_nodes',
        'operational_warning_nodes',
        'operational_blocked_nodes',
        'Next actions',
        'operator_next_action_counts',
        'operator_next_action_label_counts',
        'primary_next_action_code',
        'primary_next_action_label',
        'primary_next_action_count',
        'primary_next_action_summary',
        'primary_next_action_tone',
        'action_required',
        'Agent coverage',
        'Platform revision',
        'Host updates',
        'Agent systemd',
        'Load deployment summary for MTA agent systemd status.',
        'missing revision',
        'config drift',
        'agent_log_bounce_nodes',
        'agent_log_deferred_nodes',
        'agent_log_warning_nodes',
        'Postfix logs',
        'Load deployment summary for recent Postfix log health.',
        'Queue pressure',
        'Deferred queue',
        'Load deployment summary for deferred queue status.',
        'Provider blockers',
        'provider_port25_blocked_count',
        'provider_rdns_blocked_count',
        'provider_inactive_count',
        'submission_credentials_configured',
        'node_endpoint_label',
        'submission_endpoint_label',
        'pool_membership_label',
        'provider_blockers',
        'provider_blocker_labels',
        'provider blockers',
        'buildManagedSmtpPoolControlsEvidencePack',
        'copyManagedSmtpPoolControlsEvidencePack',
        'Managed SMTP Pool Controls Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Copied managed SMTP pool controls evidence pack',
        'operator_next_action_code',
        'operator_next_action_code_label',
        'operator_next_action',
        'operator_next_action_tone',
        'routeRecordId',
        'routeAttemptId',
        'focusedDeliveryAttempt',
        'deliveryFilterValue',
        "String(value ?? '').trim()",
        'deliveryAttemptAuditLabel',
        'deliveryAttemptAuditButtonLabel',
        'Load linked audit',
        'deliveryAttemptEmptyTitle',
        'No linked delivery attempts visible',
        'deliveryAttemptEmptyDetail',
        'proof-send submission evidence',
        'Focused attempt',
        'load the linked record audit',
        'isFocusedAttempt',
        'Focused delivery record',
        'refreshDeliveryAttempts().catch',
        'routeBlockReason',
        'smtpReason',
        'smtpResponse',
        'metadata.smtp_response',
        'attemptHasIssue',
        'attemptCompletedOk',
        'mtaHost',
        'submissionProvider',
        '<dt>submission</dt>',
        'smtpCode',
        'dkimSelector',
        'envelopeFrom',
        'action code',
        'Resolve provider blocker(s):',
        'readiness_summary_label',
        'formatManagedSmtpProviderBlocker',
        'Port 25 blocked',
        'rDNS blocked',
        'Provider inactive',
        'agent_heartbeat_status',
        'agent_heartbeat_status_label',
        'agent_operational_status',
        'agent_operational_status_label',
        'agent_last_heartbeat_label',
        'agent_heartbeat_age_seconds',
        'agent_heartbeat_stale_after_seconds',
        'agent_queue_depth',
        'agent_deferred_count',
        'agent_active_count',
        'agent_queue_status',
        'agent_queue_status_label',
        'agent_queue_samples',
        'Queue samples',
        'No queue samples reported by the MTA agent.',
        'agent_log_samples',
        'agent_log_issue_status',
        'agent_log_issue_status_label',
        'managedSmtpNodeHasLogIssue',
        'recent logs',
        'Review recent Postfix log samples and resolve delivery issues before increasing volume.',
        'Postfix log samples',
        'No Postfix log samples reported by the MTA agent.',
        'platform_config_version',
        'agent_config_version',
        'agent_applied_config_version',
        'agent_config_in_sync',
        'agent_config_sync_label',
        'agent_service_active_state',
        'agent_service_sub_state',
        'agent_service_state_label',
        'agent_timer_active_state',
        'agent_timer_sub_state',
        'agent_timer_state_label',
        'agent_timer_next_elapse',
        'agent_code_revision',
        'agent_code_dirty',
        'agent_code_revision_label',
        'agent_host_update_required',
        'agent_host_update_status',
        'agent_host_update_status_label',
        'agent_host_update_detail',
        'host update',
        'host update status',
        'host update detail',
        'managedSmtpNodeAgentServiceFailed',
        'managedSmtpNodeAgentTimerUnhealthy',
        'managedSmtpNodeHasDeferredQueue',
        'agent service',
        'agent timer',
        'code_missing_nodes',
        'code_dirty_nodes',
        'code_outdated_nodes',
        'host_update_required_nodes',
        'host update',
        'agent_service_failed_nodes',
        'agent_timer_unhealthy_nodes',
        'service failed',
        'timer unhealthy',
        'config_drift_nodes',
        'agent heartbeat',
        'node status',
        'runtime config',
        'runtime apply',
        'runtime_config_apply',
        'runtime_config_detail',
        'Runtime config apply events',
        'No runtime config apply result loaded',
        'Load runtime_config_applied MTA events to inspect generated file and reload evidence.',
        'apply result',
        'apply detail',
        'Runtime config evidence',
        'platform version',
        'agent reported',
        'agent applied',
        'last heartbeat',
        'service state',
        'timer state',
        'timer next run',
        'expected host revision',
        'host revision',
        'revision_missing',
        'dirty',
        'outdated',
        'managedSmtpNodeNextAction',
        'next action',
        'Restart or manually run the MTA agent on the host, then reload deployment summary.',
        'Restart the MTA agent service and inspect the service journal before routing traffic.',
        'Restart the MTA agent timer so recurring heartbeat and config checks continue.',
        'Resolve the host working-tree changes before using this MTA for production traffic.',
        'Run the host update workflow so this MTA pulls the deployed platform revision.',
        'Run the host update workflow so this MTA reports its code revision.',
        'Run the MTA agent once so it fetches and applies the latest runtime config.',
        'Run managed SMTP readiness smoke and publish the result before routing traffic.',
        'Inspect the MTA mail queue and resolve deferred delivery before increasing volume.',
        'No operator action required for this MTA node.',
        'managedSmtpAgentCommands',
        'managedSmtpHostUpdateCommands',
        'Host agent commands',
        'Host update workflow',
        'cd /root/apps/email-engine',
        'git pull --ff-only',
        'python3 -m py_compile scripts/managed_smtp_mta_agent.py',
        'sudo systemctl restart email-engine-mta-agent.timer',
        '# Then click Load SMTP Deployment and confirm host update returns not required.',
        'service state',
        'timer state',
        'sudo systemctl start email-engine-mta-agent.service',
        'sudo systemctl status email-engine-mta-agent.service --no-pager',
        'sudo systemctl list-timers email-engine-mta-agent.timer',
        'sudo journalctl -u email-engine-mta-agent.service -n 80 --no-pager',
        'Submission auth',
        'worker-to-MTA submission',
        'readinessChecks',
        'readinessCheckTotal',
        'readinessSummary',
        'readinessTrend',
        'readinessAlerts',
        'readinessNotification',
        'readinessFilters',
        'buildManagedSmtpReadinessEvidencePack',
        'copyManagedSmtpReadinessEvidencePack',
        'exportManagedSmtpReadinessEvidenceCsv',
        'Managed SMTP Readiness Evidence Pack',
        'Copied managed SMTP readiness evidence pack',
        'Downloaded managed SMTP readiness CSV',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'buildManagedSmtpReadinessAlertEvidencePack',
        'copyManagedSmtpReadinessAlertEvidencePack',
        'Managed SMTP Readiness Alert Evidence Pack',
        'Copied managed SMTP readiness alert evidence pack',
        'exportManagedSmtpMaintenanceCsv',
        'Export Maintenance CSV',
        'Downloaded managed SMTP maintenance CSV',
        'managed-smtp-maintenance-',
        'warmup_gate_evidence_key',
        'maintenanceBlocklistHits',
        'managedSmtpMaintenanceFilter',
        'maintenanceFilteredResults',
        'maintenanceSelectedFilter',
        'maintenanceFilterSummary',
        'Showing ${formatInt(maintenanceFilteredResults.length)} of ${formatInt(maintenanceResults.length)} policy row(s) for ${maintenanceSelectedFilter.label}. ${maintenanceSelectedFilter.detail}',
        'Run maintenance to populate filterable policy result rows.',
        'Show every policy row from the latest maintenance run.',
        'Show policy rows where warmup automation held volume.',
        'Show policy rows with one or more blocklist hits.',
        'Show policy rows skipped by maintenance automation.',
        'Show warmup rows missing stored gate evidence keys.',
        'Visible filter',
        'Export scope',
        'Full maintenance run',
        'Visible filter is included for operator context only',
        'Export scope: Full maintenance run; visible filter is included for operator context only',
        'visible filter ${maintenanceSelectedFilter.label} matched',
        'Filtered policy results',
        'maintenanceCsvRow',
        'Full maintenance run policy results',
        'no policy rows match the selected maintenance filter',
        'Managed SMTP maintenance result filters',
        'Run maintenance to populate filterable policy result rows.',
        'Show warmup rows missing stored gate evidence keys.',
        'Missing Gate',
        'The selected maintenance result filter has no policy rows in the latest run.',
        'Latest managed SMTP maintenance results',
        'No warmup action',
        'Run managed SMTP maintenance to inspect policy-level blocklist, warmup, and gate evidence outcomes.',
        'loadEvidenceReadinessChecks',
        "status: summaryTopBlockCode.count ? 'failed'",
        'domain: summaryTopSenderDomain.count ? summaryTopSenderDomain.label',
        'alert_status',
        'alert_reasons',
        'alert_checks',
        'alert_count',
        'should_notify',
        'dedupe_key',
        'feedbackFilters',
        'loadEvidenceProviderFeedbackEvents',
        'provider: summaryTopProvider.count ? summaryTopProvider.label',
        'buildProviderFeedbackEvidencePack',
        'copyProviderFeedbackEvidencePack',
        'exportProviderFeedbackEvidenceCsv',
        'Provider Feedback Evidence Pack',
        'Provider Feedback Evidence Export',
        'Copied provider feedback evidence pack',
        'Downloaded provider feedback evidence CSV',
        'Selected job proof route',
        'Selected record route',
        'previewEvidenceRouteMatrix',
        'evidence replay',
        'rule=${summaryTopRule.label}',
        'pool=${summaryTopPool.label}',
        'provider=${summaryTopProvider.label}',
        'buildManagedSmtpRouteMatrixCases(inputOverride',
        'String(inputOverride || managedSmtpRouteMatrixInput)',
        'onClick={() => previewManagedSmtpRouteMatrix()}',
        'previewAwsSecondaryRouteMatrix',
        'Run AWS Matrix',
        'AWS secondary rehearsal',
        'Load pool controls or apply aws-port25-open before running the AWS secondary matrix.',
        'buildManagedSmtpRouteMatrixPack',
        'copyManagedSmtpRouteMatrix',
        'exportManagedSmtpRouteMatrixCsv',
        'Managed SMTP Resolver Matrix',
        'Copied managed SMTP resolver matrix',
        'Downloaded managed SMTP resolver matrix CSV',
        'buildManagedSmtpRouteResolutionEvidencePack',
        'copyManagedSmtpRouteResolutionEvidencePack',
        'Managed SMTP Route Resolution Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Copied managed SMTP route resolution evidence pack',
        'draftManagedSmtpRoutingRuleFromEvidence',
        'draftAwsSecondaryRoutingRule',
        'Draft AWS Secondary',
        'aws-secondary-internal-test',
        'aws-port25-open-test',
        'Drafted disabled AWS secondary routing rule',
        'managedSmtpRoutingRuleDraftEvidence',
        'setManagedSmtpRoutingRuleDraftEvidence',
        'buildManagedSmtpRoutingRuleDraftPack',
        'copyManagedSmtpRoutingRuleDraft',
        'parseManagedSmtpRoutingRuleDraftEvidence',
        'buildManagedSmtpRoutingRuleDraftReview',
        '!managedSmtpRoutingRuleDraftEvidence',
        'Managed SMTP Routing Rule Draft',
        'Copied managed SMTP routing rule draft evidence',
        'Evidence drift',
        'Current draft fields match send type',
        'Drafted routing rule',
        'review and save it to apply',
        'providerFeedbackWarningCount',
        'readinessWarningCount',
        'latestReadinessCheck',
        'latestSuccessfulReadiness',
        'Managed SMTP Readiness',
        'readiness-status-strip',
        'readiness-status-item',
        'Managed SMTP readiness status strip',
        'Managed SMTP Deployment',
        'Managed SMTP deployment score strip',
        'Managed SMTP First Send',
        'Go/no-go checklist for the first controlled seed email through the owned MTA path.',
        'First-send status',
        'Evidence required before next seed send',
        'Ready for controlled seed send',
        'firstSendSummaryDetail',
        'firstSendControlProgress',
        'created_at',
        'Provider port 25',
        'Load Scaleway or apply the AWS port-25-open profile; AWS still needs DNS/rDNS before activation.',
        'PTR/rDNS',
        'Submission auth',
        'Domain auth',
        'MTA smoke',
        'Load First Send Evidence',
        'Load Readiness',
        'firstSendReadiness',
        'firstSendReadinessItems',
        'buildManagedSmtpFirstSendEvidencePack',
        'copyManagedSmtpFirstSendEvidencePack',
        'Managed SMTP First Send Evidence Pack',
        'Copied managed SMTP first-send evidence pack',
        'Managed SMTP Controlled Seed Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'buildManagedSmtpControlledSeedEvidencePack',
        'copyManagedSmtpControlledSeedEvidencePack',
        'Copy Controlled Seed Pack',
        'Controlled seed',
        'Seed pool gate',
        'Confirm Seed pool gate is Ready.',
        'All loaded first-send and pool gates are ready',
        'Send one internal seed recipient only.',
        'first-send-checklist',
        'first-send-check',
        'function loadFirstSendEvidence',
        '/api/v1/managed-smtp/first-send-readiness',
        'port25_status',
        'rdns_status',
        'node_status_label',
        'provider_account_status_label',
        'provider_port25_status_label',
        'provider_rdns_status_label',
        'support_case_ref',
        'support case',
        'port 25',
        'rDNS',
        'Control-plane inventory for provider accounts, MTA nodes, IP pools, routes, and node readiness.',
        'Load SMTP Deployment',
        'Pause MTA Node',
        'Resume MTA Node',
        'MTA node status change cancelled; reason is required.',
        'Operator pausing MTA node for delivery review',
        'Operator resuming MTA node after review',
        "operator: 'esp_admin'",
        'function setManagedSmtpNodeOperationalStatus',
        '/api/v1/managed-smtp/nodes/${node.id}/${action}',
        'Load MTA Events',
        'MTA Agent Telemetry',
        'Signed node events from managed MTA hosts',
        'Apply MTA Event Filters',
        'Clear MTA Event Filters',
        'Inspect Runtime Apply',
        'function inspectRuntimeConfigApplyEvents',
        'Loaded runtime config apply telemetry for',
        'No MTA agent telemetry loaded',
        'MTA event payload',
        'runtime_config_applied',
        'function loadMtaNodeEvents',
        'function updateMtaNodeEventFilter',
        '/api/v1/managed-smtp/node-events/list',
        'Submission auth',
        'No managed SMTP deployment summary loaded',
        'function loadManagedSmtpDeploymentSummary',
        '/api/v1/managed-smtp/deployment-summary',
        'Published MTA smoke, STARTTLS, DKIM, and feedback-loop checks',
        'Load SMTP Readiness',
        'Apply Readiness Filters',
        'Clear Readiness Filters',
        'Copy Readiness Pack',
        'Latest check',
        'Latest pass',
        'Passing checks',
        'Backend readiness summary across matching checks',
        'Recent trend',
        'Trend alert',
        'Alert evidence',
        'Notification payload',
        'Ready to notify',
        'Notification quiet',
        'Load readiness to prepare external alert payload.',
        'Load readiness to classify managed SMTP health.',
        'Window comparison',
        'Latest window failure rate versus previous window',
        'No managed SMTP readiness checks loaded',
        '/api/v1/managed-smtp/readiness-checks/list',
        '/api/v1/managed-smtp/readiness-checks/summary',
        '/api/v1/managed-smtp/readiness-checks/trend',
        '/api/v1/managed-smtp/readiness-checks/alerts',
        '/api/v1/managed-smtp/readiness-checks/notification',
        'function loadReadinessChecks',
        'function updateReadinessFilter',
        'Provider Feedback Evidence',
        'Retained raw provider and managed-SMTP feedback',
        'Load Provider Feedback',
        'Copy Feedback Pack',
        'Use Selected Record',
        'No provider feedback evidence loaded',
        'provider-feedback-panel',
        'provider-feedback-list',
        'provider-feedback-strip',
        'provider-feedback-strip-item',
        'Managed SMTP provider feedback strip',
        '/api/v1/provider-feedback-events/list',
        'function loadProviderFeedbackEvents',
        'function useSelectedRecordForFeedbackFilters',
        'managed_smtp_dsn_feedback',
        'dsn_bounce',
        'Retry policy',
        'Retries need provider-aware backoff, max-attempt policy, suppression checks, and permanent-failure promotion.',
        'Bounce classification',
        'Failures must classify hard bounce, soft bounce, complaint, deferral, and policy block outcomes.',
        'Feedback ingestion',
        'Provider events and managed-MTA logs feed one durable stream into suppression and analytics records.',
        'Deliverability controls',
        'Domain warmup, reputation monitoring, inbox-placement signals, and throttle overrides remain roadmap controls.',
        'domain-compliance-panel',
        'domain-compliance-strip',
        'domain-compliance-item',
        'Managed SMTP domain compliance strip',
        'Managed SMTP Domain Compliance',
        'Compliance hold, release, reputation dashboard, and policy audit controls for managed sending domains.',
        'Load Domain Policies',
        'Route mode',
        'MTA Pool',
        'Scaleway MTA',
        '3rd Party Provider',
        'MTA pool ID',
        'mta_ip_pool_id',
        'optional pool UUID',
        "selectedDomainRouteMode?.mta_ip_pool_id",
        'Save Route Mode',
        'Saving domain route mode',
        'Saved route mode',
        'Domain route mode switcher',
        'Route mode readiness summary',
        'Pool balanced',
        'Direct owned MTA',
        'Provider adapter',
        'Managed SMTP route',
        'Third-party route',
        'Only non-managed routes are available in the delivery route picker.',
        'Only managed SMTP routes are available in the delivery route picker.',
        'IP pool is not sent when saving a third-party route mode.',
        'delivery_route_mode_audit_log',
        'Route mode switch',
        'last switch',
        '/route-mode',
        'switch_to_',
        'Managed SMTP Route Resolution',
        'Resolve Route',
        'Resolve SMTP Route',
        'Copy Route Pack',
        'Decision',
        'Rule',
        'decision_basis',
        'routing_rule_name',
        'routing_rule_source',
        'preferred_providers',
        'provider_preference_mode',
        'fallback_allowed',
        'ip_pool_selection_source',
        'mta_node_candidate_count',
        'mta_node_skipped_count',
        'provider_port25_not_ready',
        'provider_rdns_not_ready',
        'Failover selection',
        'managedSmtpRouteSelection',
        'Route block details',
        'managedSmtpRouteResolution',
        'managedSmtpRouteItems',
        'managed-smtp-route-inspector',
        'managed-smtp-route-field',
        'Managed SMTP Routing Rules',
        'managedSmtpRoutingRuleForm',
        'managedSmtpRoutingRules',
        'conflicts',
        '<span>Conflict</span>',
        'Rule overlap',
        'managedSmtpRulePreview',
        'selectedManagedSmtpRoutingRuleName',
        'loadManagedSmtpRoutingRuleIntoForm',
        'function loadManagedSmtpRoutingRules',
        'function saveManagedSmtpRoutingRule',
        'function previewManagedSmtpRoutingRule',
        'function previewManagedSmtpRoutingRulePromotion',
        'function draftManagedSmtpRoutingRulePromotion',
        'function activateManagedSmtpRoutingRuleDraft',
        'function rollbackManagedSmtpRoutingRules',
        '/promotion/preview',
        '/promotion/draft',
        '/promotion/activate',
        '/promotion/rollback',
        'function setSelectedManagedSmtpRoutingRuleEnabled',
        'function deleteSelectedManagedSmtpRoutingRule',
        '/api/v1/delivery-routes/${selectedDomainPolicy.route_id}/managed-smtp/routing-rules',
        '/${encodeURIComponent(ruleName)}/${enabled ? \'enable\' : \'disable\'}',
        'Delete Rule',
        'Disable Rule',
        'Enable Rule',
        'Preview Rule',
        'Preview Promotion',
        'Draft Promotion',
        'Activate Draft',
        'Rollback Rules',
        'Managed SMTP routing rule promotion review',
        'Promotion status',
        'Blocking issue',
        'Promotion warning',
        'Draft rules',
        'Previewing managed SMTP routing rule',
        'Previewing managed SMTP routing rule promotion',
        'Drafting managed SMTP routing rule promotion',
        'Activating managed SMTP routing rule draft',
        'Rolling back managed SMTP routing rules',
        'Preview resolved',
        'Preview blocked',
        'Save Routing Rule',
        'Load Rules',
        'ManagedSmtpRouteResolutionRead',
        'function resolveManagedSmtpRoute',
        '/api/v1/managed-smtp/resolve-route',
        'Load Reputation Dashboard',
        'Apply Compliance Hold',
        'Release Compliance Hold',
        'buildManagedSmtpDomainComplianceEvidencePack',
        'copyManagedSmtpDomainComplianceEvidencePack',
        'Managed SMTP Domain Compliance Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Copied managed SMTP domain compliance evidence pack',
        'function loadDomainReputationDashboard',
        'function applyDomainComplianceHold',
        'function releaseDomainComplianceHold',
        '/api/v1/domain-delivery-policies/list',
        '/api/v1/domain-delivery-policies/${selectedDomainPolicyId}/reputation-dashboard',
        '/api/v1/domain-delivery-policies/${selectedDomainPolicyId}/compliance-hold',
        '/api/v1/domain-delivery-policies/${selectedDomainPolicyId}/release-compliance-hold',
        'delivery-foundation-panel',
        'delivery-foundation-strip',
        'delivery-foundation-item',
        'Send engine foundation strip',
        'delivery-operations-panel',
        'delivery-operations-checklist',
        'delivery-operations-check',
        'delivery-triage-panel',
        'delivery-triage-list',
        'delivery-triage-row',
        'Delivery triage support list',
        'aiDeliverySummary',
        'aiDeliveryRecommendations',
        'function reviewDeliveryWithAi',
        'function deliveryAiRecommendationAction',
        'summaryTopPool.count',
        'summaryTopRule.count && selectedDomainPolicy?.route_id',
        'summaryTopProvider.count',
        'function deleteRecord',
        '/api/v1/ai/delivery/analyze',
        '/api/v1/email-send-records/${selectedRecordId}',
        "method: 'DELETE'",
        'delivery_context',
        'AI Delivery Review',
        'Delete Record',
        'Dead-letter Record',
        'Delivery Attempt Audit',
        'Load Attempt Audit',
        'Run AI Review',
        'Load Route Evidence',
        'attemptRoutePoolValue',
        'attemptRoutePoolLabel',
        'attemptRouteProviderValue',
        'attemptRouteProviderLabel',
        'Focus Pool Controls',
        'Select Matched Rule',
        'Filter Provider Feedback',
        'Replay Resolver Matrix',
        'delivery-ai-review-panel',
        'delivery-ai-summary',
        'AI delivery route evidence',
        'Job route:',
        'Record route:',
        'Attempt evidence:',
        'No AI delivery review loaded',
    ]

    for token in expected_tokens:
        assert token in source


def test_delivery_source_has_triage_panel() -> None:
    assert_delivery_triage_contract(frontend_source())


def test_built_esp_bundle_includes_delivery_triage_panel() -> None:
    bundle = frontend_bundle()
    expected_tokens = [
        'Delivery triage',
        'Review failed records',
        'Process queued delivery',
        'Load job progress',
        'Delivery clear',
        'Queue pressure',
        'Failure review',
        'Selected job',
        'Selected record',
        'Send Engine Foundation',
        'Managed MTA nodes',
        'Send queues',
        'Bounce queues',
        'Deliverability feedback',
        'Send Engine Operations Contract',
        'SMTP service contract',
        'Queue lifecycle',
        'Retry policy',
        'Bounce classification',
        'Feedback ingestion',
        'Deliverability controls',
        'Managed SMTP Domain Compliance',
        'Load Domain Policies',
        'Load Reputation Dashboard',
        'Apply Compliance Hold',
        'Release Compliance Hold',
        'Copy Compliance Pack',
        'Managed SMTP Domain Compliance Evidence Pack',
        'Copied managed SMTP domain compliance evidence pack',
        'Managed SMTP Route Resolution Evidence Pack',
        'Copied managed SMTP route resolution evidence pack',
        'Compliance audit',
        'Provider settings',
        'Open Compliance',
        'AI Delivery Review',
        'Delete Record',
        'Dead-letter Record',
        'Delivery Attempt Audit',
        'Load Attempt Audit',
        'Run AI Review',
        'Job route:',
        'Record route:',
        'Attempt evidence:',
        'Provider Feedback Evidence',
        'Managed SMTP Readiness',
        'Managed SMTP Deployment',
        'Copy Deployment Pack',
        'Copy Fleet Pack',
        'Copy Bootstrap Pack',
        'Managed SMTP Deployment Evidence Pack',
        'Copied managed SMTP deployment evidence pack',
        'Managed SMTP Fleet Health Evidence Pack',
        'Copied managed SMTP fleet health evidence pack',
        'Managed SMTP Bootstrap Profile Evidence Pack',
        'Copied managed SMTP bootstrap profile evidence pack',
        'Run Maintenance',
        'Managed SMTP maintenance strip',
        'Maintenance processed',
        'Warmup automation',
        'Gate evidence keys',
        'Ran maintenance:',
        'Copy Maintenance Pack',
        'Export Maintenance CSV',
        'Managed SMTP Maintenance Evidence Pack',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Copied managed SMTP maintenance evidence pack',
        'Downloaded managed SMTP maintenance CSV',
        'managed-smtp-maintenance-',
        'warmup_gate_evidence_key',
        'Visible filter',
        'Export scope',
        'Full maintenance run',
        'Visible filter is included for operator context only',
        'Export scope: Full maintenance run; visible filter is included for operator context only',
        'Filtered policy results',
        'Full maintenance run policy results',
        'no policy rows match the selected maintenance filter',
        'Managed SMTP maintenance result filters',
        'Missing Gate',
        'The selected maintenance result filter has no policy rows in the latest run.',
        'Latest managed SMTP maintenance results',
        'No warmup action',
        'Run managed SMTP maintenance to inspect policy-level blocklist, warmup, and gate evidence outcomes.',
        'Run managed SMTP maintenance before copying a maintenance pack.',
        'Copy Provider Pack',
        'Managed SMTP Provider Readiness Evidence Pack',
        'Copied managed SMTP provider readiness evidence pack',
        'AWS activation signal',
        'AWS activation:',
        'AWS instance setup signal',
        'AWS agent setup:',
        'Log into AWS instance',
        'AWS agent setup commands',
        'Fleet status',
        'Route-ready nodes',
        'Agent coverage',
        'Copy Pool Pack',
        'Managed SMTP Pool Controls Evidence Pack',
        'Copied managed SMTP pool controls evidence pack',
        'Selected pool operator audit',
        'Selected membership operator audit',
        'Operator audit entries',
        'operator_audit_entries',
        'Platform revision',
        'Host updates',
        'Agent systemd',
        'config drift',
        'Deferred queue',
        'Runtime config evidence',
        'next action',
        'Host agent commands',
        'Host update workflow',
        'git pull --ff-only',
        'host revision',
        'expected host revision',
        'revision_missing',
        'outdated',
        'Run the host update workflow so this MTA pulls the deployed platform revision.',
        'sudo systemctl start email-engine-mta-agent.service',
        '# Then click Load SMTP Deployment and confirm host update returns not required.',
        'Run the MTA agent once so it fetches and applies the latest runtime config.',
        'Queue pressure',
        'Provider blockers',
        'Pause MTA Node',
        'Resume MTA Node',
        'MTA Agent Telemetry',
        'Load MTA Events',
        'Apply MTA Event Filters',
        'Clear MTA Event Filters',
        'Copy MTA Events',
        'Copy Runtime Apply Pack',
        'Export Runtime Apply CSV',
        'Export MTA Events CSV',
        'MTA Agent Telemetry Evidence Pack',
        'Runtime Config Apply Evidence Pack',
        'Load runtime_config_applied MTA events before copying runtime config apply evidence.',
        'Load runtime_config_applied MTA events before exporting runtime config apply CSV.',
        'Runtime Config Apply Evidence Export',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'runtime-config-apply-evidence-',
        'Loaded apply events',
        'Loaded apply rows',
        'Apply events',
        'Copied MTA agent telemetry evidence pack',
        'Copied runtime config apply evidence pack',
        'Downloaded runtime config apply CSV',
        'Downloaded MTA agent telemetry CSV',
        'No MTA agent telemetry loaded',
        'runtime_config_applied',
        'Managed SMTP First Send',
        'First-send status',
        'Evidence required before next seed send',
        'Provider port 25',
        'Load Scaleway or apply the AWS port-25-open profile; AWS still needs DNS/rDNS before activation.',
        'PTR/rDNS',
        'Domain auth',
        'MTA smoke',
        'Load First Send Evidence',
        'Load SMTP Deployment',
        'Copy First-Send Pack',
        'Managed SMTP First Send Evidence Pack',
        'Copied managed SMTP first-send evidence pack',
        'Seed pool gate',
        'Confirm Seed pool gate is Ready.',
        'No managed SMTP deployment summary loaded',
        'support case',
        'port 25',
        'rDNS',
        'Load SMTP Readiness',
        'Apply Readiness Filters',
        'Clear Readiness Filters',
        'Copy Readiness Pack',
        'Export Readiness CSV',
        'Copy Alert Pack',
        'Managed SMTP Readiness Evidence Pack',
        'Copied managed SMTP readiness evidence pack',
        'Downloaded managed SMTP readiness CSV',
        'Managed SMTP Readiness Alert Evidence Pack',
        'Copied managed SMTP readiness alert evidence pack',
        'Trend alert',
        'Alert evidence',
        'Notification payload',
        'Ready to notify',
        'Notification quiet',
        'Load readiness to prepare external alert payload.',
        'Load readiness to classify managed SMTP health.',
        'No managed SMTP readiness checks loaded',
        'Load Provider Feedback',
        'Use Selected Record',
        'No provider feedback evidence loaded',
        'Retained raw provider and managed-SMTP feedback',
        '/api/v1/provider-feedback-events/list',
        'Load Profiles',
        'Apply Profile',
        'Profile applied',
        'Provider bootstrap profile',
        '/api/v1/managed-smtp/bootstrap-profiles/list',
        '/api/v1/managed-smtp/bootstrap-profiles/',
        '/api/v1/managed-smtp/deployment-summary',
        '/api/v1/managed-smtp/node-events/list',
        '/api/v1/managed-smtp/first-send-readiness',
        '/api/v1/managed-smtp/readiness-checks/list',
        '/api/v1/managed-smtp/readiness-checks/summary',
        '/api/v1/managed-smtp/readiness-checks/trend',
        '/api/v1/managed-smtp/readiness-checks/alerts',
        '/api/v1/managed-smtp/readiness-checks/notification',
        'managed_smtp_dsn_feedback',
        'dsn_bounce',
        '/api/v1/delivery-attempts/list',
        '/api/v1/delivery-attempts/evidence-summary',
        '/api/v1/delivery-attempts/evidence-export.csv',
        'Route Blocks',
        'Resolved Rule Hits',
        'Strict Provider Preference',
        'delivery attempt evidence preset.',
        'Apply Attempt Filters',
        'Clear Attempt Filters',
        'mta_route_resolved',
        'mta_route_send_type',
        'mta_route_sender_domain',
        'mta_route_recipient_domain',
        'mta_routing_rule_name',
        'mta_routing_rule_source',
        'mta_rule_hit_pool_source',
        'mta_rule_hit_provider_preference',
        'mta_rule_hit_provider_preference_mode',
        'mta_provider_preference_fallback_used',
        'mta_node_skipped_reason',
        'Preference Fallbacks',
        'Readiness Skips',
        'Preference mode',
        'Fallback used',
        'Skipped reason',
        'top_provider_preferences',
        'top_provider_preference_modes',
        'top_provider_fallbacks',
        'top_provider_preference_blocks',
        'top_provider_fallback_availability',
        'Top provider preference',
        'Top preference mode',
        'Fallback usage',
        'Preference block state',
        'Fallback availability',
        'provider_preference_blocked',
        'provider_fallback_available',
        'Submission provider',
        'mta_route_block_code',
        'Active delivery attempt evidence filters',
        'Active evidence filters',
        'Remove filter',
        'Removed ',
        'Cleared delivery attempt evidence filters.',
        'Auto-refreshed delivery attempt evidence filters',
        'Delivery attempt evidence rollup',
        'Evidence Follow-Up',
        'Jump from route evidence into pool, rule, provider, and readiness controls.',
        'Inspect selected pool',
        'Focus Pool Controls',
        'Loaded evidence-matched pool controls',
        'Review matched rule',
        'Select Matched Rule',
        'Loaded and selected evidence-matched routing rule',
        'Draft evidence rule',
        'Draft Rule From Evidence',
        'Prefill a routing rule from evidence dimensions',
        'Draft evidence',
        'No evidence draft loaded',
        'Preview Draft',
        'Copy Draft Evidence',
        'Managed SMTP Routing Rule Draft',
        'Copied managed SMTP routing rule draft evidence',
        'Managed SMTP routing rule draft review',
        'Draft Review',
        'Evidence match',
        'Evidence drift',
        'Validate provider path',
        'Review provider feedback',
        'Filter Provider Feedback',
        'Load retained feedback with the route provider filter applied',
        'Copy Feedback Pack',
        'Export Feedback CSV',
        'Provider Feedback Evidence Pack',
        'Copied provider feedback evidence pack',
        'Downloaded provider feedback evidence CSV',
        'Replay route case',
        'Replay Resolver Matrix',
        'Fill the resolver matrix from the top evidence dimensions',
        'Copy Matrix',
        'Export Matrix CSV',
        'Managed SMTP Resolver Matrix',
        'Copied managed SMTP resolver matrix',
        'Downloaded managed SMTP resolver matrix CSV',
        'Selected route proof',
        'Selected job proof route',
        'Selected record route',
        'Resolve block evidence',
        'Matching evidence rows',
        'Export scope',
        'Export cap',
        'Full matching set fits within the CSV export cap.',
        'Copy Evidence Pack',
        'Delivery Attempt Evidence Pack',
        'Selected route proof',
        'Selected job proof route:',
        'Selected record route:',
        'Selected record route attempt:',
        'Copied delivery attempt evidence pack',
        'Controlled Seed Proof',
        'Controlled seed proof',
        'Controlled expansion',
        'Expansion pool gate',
        'Feedback gate',
        'Load retained provider feedback before advancing warmup volume.',
        'Warmup review',
        'Managed SMTP Warmup Review Pack',
        'Selected route proof',
        'Copied managed SMTP warmup review pack',
        'Copy Warmup Review',
        'Evaluate Warmup',
        'Advance Warmup',
        'Warmup progression',
        'Warmup audit',
        'Warmup gate evidence',
        'Evaluate or advance warmup to store proof, pool, feedback, and automation gate evidence.',
        'Warmup audit log',
        'Warmup audit entries',
        'Warmup last evaluated',
        'Warmup hold reason',
        'Gate evidence snapshot',
        'gate_evidence',
        'gate_evidence_keys',
        'gate_evidence_summary',
        'gates=',
        'metrics_ready',
        'controlled_seed_proof',
        'Evaluated warmup for',
        'Advanced warmup for',
        '/warmup-progress',
        'Warmup review is not advance-ready',
        'Warmup decision',
        'Advance ready',
        'This pack is read-only and does not call warmup-progress.',
        'Managed SMTP Controlled Expansion Review Pack',
        'selected_route_proof',
        'Copied managed SMTP controlled expansion pack',
        'Copy Expansion Pack',
        'Approve Expansion',
        'Approving managed SMTP controlled expansion',
        '/controlled-expansion/approve',
        'Expansion daily limit',
        'Expansion send types',
        'Expansion expires hours',
        'Expansion approval',
        'Approval result',
        'Managed SMTP controlled expansion approval',
        'Approve controlled expansion before campaign traffic can claim managed SMTP send records.',
        'controlled_expansion_approved_from_delivery_review',
        'Expansion decision',
        'Stage next cohort',
        'Warmup daily limit',
        'Pool operator audit',
        'Membership operator audit',
        'Pool operator audit entries',
        'Membership operator audit entries',
        'Managed SMTP Controlled Seed Proof Pack',
        'Copied managed SMTP controlled seed proof pack',
        'Copy Seed Proof Pack',
        'Proof decision',
        'Expand cautiously',
        'Hold expansion',
        'Export Evidence CSV',
        'delivery-attempt-evidence-',
        'scope and truncation metadata',
        'selected_job_proof_route',
        'selected_record_route',
        'selected_record_route_attempt_id',
        'Top route status',
        'Top resolved state',
        'top_route_statuses',
        'top_route_resolved_states',
        'route_status',
        'Top route provider',
        'Top route pool',
        'Pool pressure signal',
        'Pool pressure:',
        'Top routing rule',
        'Top send type',
        'Top sender domain',
        'Top recipient domain',
        'Top rule source',
        'Top pool source',
        'Top block code',
        'Top block message',
        'top_block_messages',
        'Provider failover signal',
        'Provider failover:',
        'Top candidate count',
        'Rule performance signal',
        'Rule performance:',
        'Top skipped count',
        'top_node_candidate_counts',
        'top_node_skipped_counts',
        'candidate_count',
        'skipped_count',
        'Top selection priority',
        'Top selection weight',
        'Submission path signal',
        'Submission path:',
        'Top submission provider',
        'Top submission host',
        'Top submission port',
        'Top MTA hostname',
        'top_node_selection_priorities',
        'top_node_selection_weights',
        'top_submission_providers',
        'top_submission_hosts',
        'top_submission_ports',
        'top_mta_hostnames',
        'selection_priority',
        'selection_weight',
        'delivery_route_mode',
        'route_mode_provider',
        'route_mode_ip_pool_name',
        'route_mode_mta_ip_pool_id',
        'route_mode_pool',
        'Job proof route',
        'submission_provider',
        'submission_host',
        'submission_port',
        'mta_hostname',
        'Top capacity status',
        'Top available nodes',
        'Top required nodes',
        'top_pool_capacity_statuses',
        'top_pool_available_node_counts',
        'top_pool_required_node_counts',
        'pool_capacity_status',
        'pool_available_node_count',
        'pool_required_node_count',
        'Top rate-limit scope',
        'Top rate limit',
        'Top recent rate count',
        'top_rate_limit_scopes',
        'top_rate_limit_max_per_minute',
        'top_rate_limit_recent_counts',
        'rate_limit_scope',
        'rate_limit_window_seconds',
        'rate_limit_max_per_minute',
        'rate_limit_recent_count',
        'Missing evidence dimension',
        'Evidence completeness signal',
        'Evidence completeness:',
        'top_missing_evidence_dimensions',
        'missing_evidence_dimensions',
        'Resolver timeline',
        'resolver_timeline',
        'rule source',
        'pool source',
        '/api/v1/ai/delivery/analyze',
        'No AI delivery review loaded',
    ]

    for token in expected_tokens:
        assert token in bundle
