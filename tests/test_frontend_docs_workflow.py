from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
FRONTEND_SOURCE = ROOT / 'frontend' / 'src' / 'main.tsx'
FRONTEND_DIST = ROOT / 'frontend' / 'dist'


def frontend_source() -> str:
    return FRONTEND_SOURCE.read_text()


def frontend_bundle() -> str:
    assets = sorted((FRONTEND_DIST / 'assets').glob('index-*.js'))
    assert assets, 'frontend/dist does not include the built ESP index bundle'
    return '\n'.join(asset.read_text() for asset in assets)


def assert_docs_triage_contract(source: str) -> None:
    expected_tokens = [
        'docsTriageAction',
        'docsTriageItems',
        "title: 'Resolve schema contract'",
        "title: 'Configure delivery contract'",
        "title: 'Document owned SMTP gap'",
        "title: 'Set public endpoints'",
        "title: 'Run live contract checks'",
        "title: 'Review failing workflow'",
        "title: 'Contract ready'",
        'Contract triage',
        'Schema',
        'Owned SMTP',
        'Public endpoints',
        'Live checks',
        'Object ownership',
        'platformGapRows',
        'Platform Gap Register',
        'Roadmap guidance for foundations not fully covered by the current API contract.',
        'Data graph',
        'RDBMS, warehouse, NoSQL, API connectors, multi-entity joins, and client-owned entities need first-class contracts.',
        'Owned SMTP',
        'SMTP server, MTA policy, send queues, throttling, and domain controls remain platform foundation work.',
        'Deliverability feedback',
        'Bounce queues, complaint loops, provider webhooks, and reputation feedback need durable event contracts.',
        'Agent layer',
        'Ever-present agents need OpenAI configuration, persistent memory, handoff state, and workflow observability.',
        'Owned SMTP remains a first-class platform foundation even when provider adapters are available.',
        'Email Engine remains system of record for ESP objects',
        'docsLifecycleItems',
        'API Lifecycle Readiness',
        'Ownership, versioning, client contract checks, and release evidence.',
        'API ownership',
        'ESP objects should remain owned by Email Engine while external clients integrate through API contracts.',
        'Versioning policy',
        'Schema migrations must be current before external client release.',
        'Client contract',
        'Run live checks before handing this contract to SentientMail or other clients.',
        'Release evidence',
        'Release evidence should capture diagnostics, smoke checks, and integration notes.',
        'docs-gap-panel',
        'docs-gap-list',
        'docs-gap-row',
        'docs-lifecycle-panel',
        'docs-lifecycle-strip',
        'docs-lifecycle-item',
        'docs-triage-panel',
        'docs-triage-list',
        'docs-triage-row',
    ]

    for token in expected_tokens:
        assert token in source


def test_docs_source_has_contract_triage_panel() -> None:
    assert_docs_triage_contract(frontend_source())


def test_built_esp_bundle_includes_contract_triage_panel() -> None:
    bundle = frontend_bundle()
    expected_tokens = [
        'Contract triage',
        'Resolve schema contract',
        'Configure delivery contract',
        'Document owned SMTP gap',
        'Set public endpoints',
        'Run live contract checks',
        'Review failing workflow',
        'Contract ready',
        'Owned SMTP',
        'Public endpoints',
        'Live checks',
        'Object ownership',
        'Platform Gap Register',
        'Data graph',
        'Deliverability feedback',
        'Agent layer',
        'API Lifecycle Readiness',
        'API ownership',
        'Versioning policy',
        'Client contract',
        'Release evidence',
        'Open Integrations',
        'Open API Docs',
    ]

    for token in expected_tokens:
        assert token in bundle
