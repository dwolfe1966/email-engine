...
def test_deployment_summary_combines_inventory_counts_and_node_readiness(monkeypatch) -> None:
    ...
    assert summary.recent_nodes[0].agent_last_heartbeat_label.endswith(' UTC')
    ...
    assert summary.fleet_health.primary_next_action_tone == 'good'
...
def test_agent_heartbeat_state_marks_old_heartbeat_stale() -> None:
    ...
    assert service._datetime_label(None) == '-'
...
def test_fleet_health_warns_when_agent_code_revision_is_missing_dirty_or_outdated(
    ...
    assert summary.operator_next_action_label_counts == {
        'Report host revision': 1,
    ...
    assert summary.primary_next_action_tone == 'warn'
